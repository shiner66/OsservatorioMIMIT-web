import { Clock, ExternalLink, Fuel, MapPin } from "lucide-react";
...
export function StationCard({ station, highlightFuel, mode, onSelect, selected, priceBadge }: Props) {
  const label = station.name || station.brand || `Impianto ${station.id}`;
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            {station.brand && <span className="text-xs text-slate-500 shrink-0">{station.brand}</span>}
          </div>
